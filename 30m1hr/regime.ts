// regime.ts
import { EMA, MACD, ADX, RSI } from 'technicalindicators';
...
// === Helpers ===
function getEMAValue(close: number[], period: number): number {
  const arr = EMA.calculate({ period, values: close });
  return arr.at(-1) ?? close.at(-1)!;
}

function getMACDHist(close: number[]): number {
...
function getADXValue(ohlcv: OHLCV, period: number): number {
...
function getRSIValue(close: number[], period = 14): number {
  const arr = RSI.calculate({ values: close, period });
  return arr.at(-1) ?? 50;
}

function getATRValue(ohlcv: OHLCV, period: number): number {
  const ranges: number[] = [];
...
export async function detectRegime(
  ...
  const volatilitySlope =
    atrPct - getATRValue(ohlcv, 28) / closeArr.at(-1)!;
  const adx = getADXValue(ohlcv, 14);
  const macdSlope =
    getMACDHist(closeArr) - getMACDHist(closeArr.slice(0, -1));
  const rsiArr = getRSIValue(closeArr);
  const rsiSlope =
    rsiArr - getRSIValue(closeArr.slice(0, -1));

  // Base confidence from combined metrics
  const trendScore =
    Math.abs(emaSlope) + Math.abs(macdSlope) + adx + Math.abs(rsiSlope);
  ...
  const regimeMap: Record<Regime, string> = {
  ...
  throw new Error(
    `No matching dynamic config for any regime on ${symbol}`
  );
}
 ...
 */
export async function guessMarketRegime(
  symbol: string,
  ohlcv30m: OHLCV,
  ohlcv1h: OHLCV
): Promise<RegimeResult> {
